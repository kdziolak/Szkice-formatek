...
$targetDir = Join-Path $runtimeRoot 'jdk-21'
$stagingDir = Join-Path $runtimeRoot '_jdk-21-extract'
$archivePath = Join-Path $cacheRoot 'temurin-21-windows-x64.zip'
...
if (Test-Path (Join-Path $verifiedTargetDir 'bin\java.exe')) {
    Write-Host "Repo-local JDK 21 is already installed at $verifiedTargetDir"
...
if (-not (Test-Path $verifiedArchivePath)) {
    Write-Host "Downloading Temurin 21 LTS to $verifiedArchivePath"
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    Invoke-WebRequest -Uri $downloadUrl -OutFile $verifiedArchivePath
}
else {
    Write-Host "Using cached Temurin 21 archive at $verifiedArchivePath"
}
...
if (Test-Path $verifiedStagingDir) {
...
$extractedJdkDir = Get-ChildItem -LiteralPath $verifiedStagingDir -Directory |
...
Move-Item -LiteralPath $extractedJdkDir.FullName -Destination $verifiedTargetDir
Remove-Item -LiteralPath $verifiedStagingDir -Recurse -Force

Write-Host "Installed repo-local JDK 21 to $verifiedTargetDir"
& (Join-Path $verifiedTargetDir 'bin\java.exe') -version
